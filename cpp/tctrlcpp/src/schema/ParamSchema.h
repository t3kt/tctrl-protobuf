...
using ParamOptionList = tctrl::schema::OptionList;
...
public:
	MenuParamSchema(const Spec& spec, const OptionListProvider* optionListProvider = nullptr)
		...
		if (spec.option_size() > 0) {
			for (const auto& option : spec.option()) {
				*(_options.add_option()) = option;
			}
		}
		else if (optionListProvider != nullptr && !spec.optionlistkey().empty()) {
			const auto options = optionListProvider->getOptionList(spec.optionlistkey());
			if (options != nullptr) {
				_options = *options;
			}
		}
